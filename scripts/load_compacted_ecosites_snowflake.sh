#!/usr/bin/env bash
# Bulk-load stg_ecosites_compacted.csv into Snowflake using snowsql PUT + COPY INTO.
set -euo pipefail

CSV="$(cd "$(dirname "$0")/.." && pwd)/dbt/seeds/stg_ecosites_compacted.csv"
DB="chaker_temp"
SCHEMA="public_arborphy"
TABLE="stg_ecosites_compacted"
